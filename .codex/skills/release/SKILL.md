...
| File | Field |
|------|-------|
| `WisprDuck.xcodeproj/project.pbxproj` | `MARKETING_VERSION` (both Debug and Release) |
| `site/index.html` | `"softwareVersion"` in the Schema.org JSON-LD |
| `site/package.json` | `"version"` |
...
xcodebuild archive \
  ...
  -archivePath /tmp/WisprDuck.xcarchive \
  CODE_SIGN_IDENTITY="Developer ID Application: Tiny Anvil, LLC (T4GBHCYB7P)" \
  DEVELOPMENT_TEAM=T4GBHCYB7P \
...
xcodebuild -exportArchive \
  -archivePath /tmp/WisprDuck.xcarchive \
  -exportPath /tmp/WisprDuckExport \
...
xcrun notarytool submit /tmp/WisprDuck-$ARGUMENTS.zip \
  --keychain-profile "WisprDuck-Notarize" \
  --wait
```
...
```
xcrun notarytool store-credentials "WisprDuck-Notarize" \
  --key ~/Desktop/wispr-duck-certs/AuthKey_4MM3YQXN45.p8 \
  --key-id 4MM3YQXN45 \
  --issuer 69a6de83-bf58-47e3-e053-5b8c7c11a4d1
```
...
gh release create v$ARGUMENTS \
  /tmp/WisprDuck-$ARGUMENTS.zip \
  --repo kalepail/wispr-duck \
  --title "WisprDuck v$ARGUMENTS" \
  --generate-notes
```
